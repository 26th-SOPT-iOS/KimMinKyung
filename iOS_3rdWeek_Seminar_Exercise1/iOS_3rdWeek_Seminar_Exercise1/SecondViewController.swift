...
class SecondViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let date4 = DateInformation(weather: .sunny, date: "5월 5일", subTitle: "어린이날")
            let date5 = DateInformation(weather: .cloudy, date: "5월 22일", subTitle: "석가탄신일"); let date6 = DateInformation(weather: .cloudy, date: "6월 6일", subTitle: "현충일")
        ...
        let date9 = DateInformation(weather: .rainny, date: "10월 3일", subTitle: "개천절")
            let date10 = DateInformation(weather: .cloudy, date: "10월 9일", subTitle: "한글날"); let date11 = DateInformation(weather: .snowy, date: "12월 26일", subTitle: "성탄절")
...
extension SecondViewController: UITableViewDataSource{
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell { guard let weatherCell = tableView.dequeueReusableCell(withIdentifier: WeatherCell.identifier, for:
    indexPath) as? WeatherCell else { return UITableViewCell() }; weatherCell.setDateInformation(weatherImgName:
...
extension SecondViewController: UITableViewDelegate{
    ...
    "DetailViewController") as? DetailViewController else { return }
        detailViewController.imageName = dateInformations[indexPath.row].weather.getImageName(); detailViewController.date = dateInformations[indexPath.row].date; detailViewController.subTitle = dateInformations[indexPath.row].subTitle; self.present(detailViewController, animated: true, completion: nil)
    }
